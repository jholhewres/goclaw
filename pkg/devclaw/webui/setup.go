...
func (s *Server) handleSetupFinalize(w http.ResponseWriter, r *http.Request) {
	...
	// Build .env: always include the vault password for non-interactive
	// unlock (PM2, Docker, systemd). If the vault failed, also include
	// secrets as plaintext fallback so the system still works.
	var envBuilder strings.Builder
	envBuilder.WriteString("# DevClaw — generated by setup wizard\n")
	if vaultOK {
		envBuilder.WriteString("# Only the vault password is stored here. All other secrets\n")
		envBuilder.WriteString("# (API keys, tokens) are encrypted in .devclaw.vault.\n")
	} else {
		envBuilder.WriteString("# WARNING: vault creation failed — secrets stored in plain text.\n")
		envBuilder.WriteString("# Re-run setup or use 'devclaw config vault-init' to encrypt.\n")
		if setup.APIKey != "" {
			envBuilder.WriteString("DEVCLAW_API_KEY=" + setup.APIKey + "\n")
		}
	}
	envBuilder.WriteString("DEVCLAW_VAULT_PASSWORD=" + vaultPassword + "\n")
